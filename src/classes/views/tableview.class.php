...
class TableView{
    public static function showEditableTable($table_data, $item_name, $itemsPerPage = 10) {
        ...
        if (count($table_data) > 0) {
            ...
            for ($i = $startIndex; $i < $endIndex; $i++) {
                ...
                $identifier = $row[$id];
                //href links to update or delete
    ...
    public static function showReadOnlyTable($table_data, $itemsPerPage = 10){
        // Get the current page number from the URL, default to 1 if not set
        $currentPage = isset($_GET['page']) ? $_GET['page'] : 1;
    ...
    public static function showSelectTable($table_data, $user, $action, $itemsPerPage = 10) {
        // Get the current page number from the URL, default to 1 if not set
        $currentPage = isset($_GET['page']) ? $_GET['page'] : 1;
